...
class UsersThrottlingController extends BaseController {
    ...
    public function getStatus($id)
    {
        try
        {
            ...
            return View::make(Config::get('backend::views.throttle_status'), compact('throttle','user'));
        }
            ...
            switch ($action)
            {
                ...
                default:
                    return Redirect::route('admin.users.index')
                        ->with('error', Lang::get('backend::throttle.action_not_found', array('action' => $action)));
                    break;
            }

            return Redirect::route('admin.users.index')
                ->with('success', Lang::get('backend::throttle.success',array('action' => $action)));
